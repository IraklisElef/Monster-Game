...
public class Wizard extends GameEntity {
	...
	public Wizard(int x, int y, float z, String name, int health, int healthPotions, int damage, int criticalChance) {
	...
	public void renderObjectAttack(Graphics g) {
		...
		if(hasShadows) {
			...
			if(delayAttack > 75 && delayAttack <= 100)
				g.drawImage(attack[0], x, y, null);
			else if(delayAttack > 50 && delayAttack <= 75)
			...
			if(delayAttack > 75 && delayAttack <= 100)
				...
				g.drawImage(attack_no_shadows[3], x, y, null);	
